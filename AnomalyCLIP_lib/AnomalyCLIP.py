        ...
        for r in self.resblocks:
            ...
            if idx in out_layers:
                if isinstance(x, list):
                    ...
                    # out_tokens.append(copy.deepcopy(x[0]))
                else:
                    out_tokens.append(x)
        return x, out_tokens
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, features_list, ori_patch=False, proj_use=True, DPAM_layer=None, ffn=False):
        ...
        if True:
            ...
            return x_ori, patch_tokens
        #########QKV##########
        # if True:
        #     patch_token_list = []
        #     for _ in range(4):
        #         patch_token = x_ori.permute(1, 0, 2) @ self.proj  # LND -> NLD
        #         #patch_token = self.ln_post(x_ori.permute(1, 0, 2)) @ self.proj
        #         patch_token_list.append(patch_token)
        #     patch_tokens = patch_token_list
        #     x_ori = x_ori[0, :, :]
        #     x_ori = x_ori @ self.proj
