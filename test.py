...
def test(args):
    ...
    for idx, items in enumerate(tqdm(test_dataloader)):
        ...
        with torch.no_grad():
            ...
            win_text_probs = (win_text_probs[:, :, 1].max()).unsqueeze(0)
            ####################################################
            ...
            w_anomaly_map = calWinAnoMap(w_anomaly_map, win_masklist, winsize, 37, device) #17,13  #[1,1,37,37]
    ...
    for obj in obj_list:
        ...
        ###########################
        pr_px_array = np.array(pr_px)
        # if np.isnan(pr_px_array).any():
        #     mean_pr_px_array = np.nanmean(pr_px_array)
        #     pr_px_array = np.nan_to_num(pr_px_array, nan=mean_pr_px_array)
        pr_px = pr_px_array
        ##################################
        pr_sp_array = np.array(pr_sp)
        # if np.isnan(pr_sp_array).any():
        #     mean_pr_sp_array = np.nanmean(pr_sp_array)
        #     pr_sp_array = np.nan_to_num(pr_sp_array, nan=mean_pr_sp_array)
        pr_sp = pr_sp_array
        ##################################
    ...
    # results = tabulate(table_ls, headers=['objects','auroc_px','aupro'], tablefmt="pipe")
    logger.info("\n%s", results)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--save_path", type=str, default='./results/mvtec_kk', help='path to save results')
    parser.add_argument("--checkpoint_path", type=str, default='./checkpoint/visa_kk/epoch_15.pth',
